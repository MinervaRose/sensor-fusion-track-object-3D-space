...
void detKeypointsModern(std::vector<cv::KeyPoint> &keypoints, cv::Mat &imgGray, std::string detectorType, bool bVis)
{
    ...
    else if (detectorType == "SIFT") detector = cv::xfeatures2d::SIFT::create();
    else if (detectorType == "HARRIS" || detectorType == "SHITOMASI") return; // already handled elsewhere
    else
    {
        std::cerr << "Detector type not recognized: " << detectorType << std::endl;
        return;
    }
    ...
    if (bVis)
    {
        ...
        cv::imshow(detectorType + " Detector", visImage);
        cv::waitKey(0);
    }
}
